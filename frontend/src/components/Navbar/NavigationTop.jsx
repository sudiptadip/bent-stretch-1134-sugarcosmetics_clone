...
import { toggleSearch, toggleUsername } from "../Homepage/Cart"
...
function NavigationTop() {
  ...
  const heart = useSelector((state) => state.cart.faveItems);
        ...
        <Flex className={styles.sugarnavtop}>
          ...
          <Box className={styles.loginregister}>
            ...
            {username !== "Login/Register" && (
              <Button
                onClick={() => {
                  dispatch(toggleUsername("Login/Register"));
                }}
          ...
          <Box className={styles.icongroup}>
            ...
            {/*    <Link   as={} size='xs' w={6} h={6} mr='6px' color='white' variant='unstyled'/> */}
            <div className={styles.quantityNum} style={{ marginRight: "25px" }}>
              <p>{getTotalFavQuantity() || 0}</p>
            </div>
            <Link to="/">
              <BiHeart
